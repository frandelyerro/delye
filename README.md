...
- Includes **ML Training Baseline v1** (`/ml-lab` → Train section): Train a transparent, local **logistic-regression** model on labeled historical outcomes. Choose a target (hydrocarbon presence / geological success / commercial success), a feature mode (safe pre-drill or expert calibration), and a train/test split. The tool builds a leakage-safe feature matrix, trains in the browser (no backend, no external API), and reports accuracy, precision, recall, F1, Brier score, a confusion matrix, and a per-prospect ML-vs-expert comparison. The trained model can be saved/loaded/cleared in `localStorage` and appears as an **advisory** prediction on each Prospect Detail page. **ML output never overrides expert-system GCoS, prospect priority, recommended action, drill-candidate logic, economics decision signals, or geoscience scores.** See [docs/ml-training-baseline.md](docs/ml-training-baseline.md).
- Includes **Norway FactPages Adapter** (`/ml-lab` → Import section): Auto-detects and converts Sokkeldirektoratet FactPages wellbore CSV exports (`wellbore_exploration_all.csv`) into PetroTarget's 28-column ML import schema. Derives outcome labels (dry_hole, technical_discovery, commercial_discovery) from the HC content column and discovery/field association. Optional enrichment with discovery, reserves, description, and field CSVs improves coverage. Note: FactPages does not publish pre-drill geological scores — all six component scores default to 0.5. See [docs/norway-factpages-adapter.md](docs/norway-factpages-adapter.md).
...
- [ML Core](docs/ml-core.md) — feature extraction, deterministic baseline preview, readiness assessment
- [ML Dataset Import](docs/ml-dataset-import.md) — 28-column CSV import schema, validation rules, leakage warnings
- [ML Training Baseline](docs/ml-training-baseline.md) — supervised logistic-regression prototype, feature modes, leakage prevention, metrics, limitations
- [Norway FactPages Adapter](docs/norway-factpages-adapter.md) — field mapping, outcome derivation, enrichment files, limitations
