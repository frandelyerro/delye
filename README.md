...
- Includes **ML Column Mapping** (`/ml-lab` → Import → Step 2): Adapts external petroleum datasets (from regulators, public repositories, or internal DBs) to the PetroTarget schema via a 4-step import flow. Auto-suggests mappings from 10+ common column aliases, supports 5 preset mappings (Generic, NSTA-like, NOPIMS-like, NPD-like, NLOG-like), normalizes outcome strings ("dry hole" → `dry_hole`, "commercial discovery" → `commercial_discovery`, etc.), and defaults non-critical columns. See [docs/ml-column-mapping.md](docs/ml-column-mapping.md).
